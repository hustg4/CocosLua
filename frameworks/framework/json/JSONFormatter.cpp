...
const char* JSONFormatter::format(Ref *object, bool pretty)
{
    ...
    __String* ccstr=__String::create(jsonChar);
    json_free(jsonChar);
    json_delete(jsonNode);
    return ccstr->getCString();
}

JSONNODE* JSONFormatter::formatToJSONNode(Ref* object)
{
...
JSONNODE* JSONFormatter::formatJSONObjectToJSONNode(JSONObject* jsonObject)
{
    ...
    __Array* allKeys=jsonObject->allKeys();
    for (int i=0;i<count;i++) {
        __String* key=(__String*)allKeys->getObjectAtIndex(i);
        Ref* obj=jsonObject->get(key->getCString());
