...
//     reader->_ownerCallbackNodes->retain();
    ...
    if (reader->isJSControlled() && pCCBReader->isJSControlled() && NULL == reader->_owner)
    {
        ...
        auto ownerCallbackControlEvents = reader->getOwnerCallbackControlEvents();//add by g4
        if (!ownerCallbackNames.empty() && !ownerCallbackNodes.empty())
        {
            ...
            for (ssize_t i = 0 ; i < nCount; i++)
            {
                ...
                pCCBReader->addOwnerCallbackControlEvents((Control::EventType)ownerCallbackControlEvents[i].asInt());//add by g4
            }
        }
        //set variables
